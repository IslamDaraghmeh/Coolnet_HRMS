...
class SimplePostmanGenerator {
  ...
  getRequestBody(requestBody, swaggerSpec) {
    try {
      ...
      console.warn(
        `Warning: Could not generate request body for operation: ${error.message}`
      );
      return {
        mode: "raw",
        raw: "{}",
  ...
  generateExampleFromSchema(schema, swaggerSpec) {
    try {
      ...
      console.warn(
        `Warning: Could not generate example from schema: ${error.message}`
      );
      return "{}";
    }
  }
  ...
  resolveRef(ref, swaggerSpec) {
    try {
      ...
      console.warn(
        `Warning: Could not resolve reference ${ref}: ${error.message}`
      );
      return null;
    }
  }
...
if (require.main === module) {
  const generator = new SimplePostmanGenerator();
  generator.generate();
}

module.exports = SimplePostmanGenerator;
